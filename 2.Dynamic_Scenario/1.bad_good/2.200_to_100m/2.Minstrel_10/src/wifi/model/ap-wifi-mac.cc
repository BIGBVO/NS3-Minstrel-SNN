...
ApWifiMac::updateMCS (std::map<Mac48Address, double> address_sinr_map){
    std::map<Mac48Address, uint16_t> mcs_index_map;
    /*
    if (received_data == false){
      for (auto it = address_sinr_map.begin(); it != address_sinr_map.end(); ++it){
        mcs_index_map[it->first] = 9999;
      }
    }else{
      for (auto it = address_sinr_map.begin(); it != address_sinr_map.end(); ++it){
        m_mcsdl->SetSINR(it->second);
        uint16_t mcs_rate = m_mcsdl->GetMCS();
        mcs_index_map[it->first] = mcs_rate;
      }
    }
    */
    for (auto it = address_sinr_map.begin(); it != address_sinr_map.end(); ++it){
        mcs_index_map[it->first] = 0;
    }
    return mcs_index_map;
}
                        ...
                        Mac48Address to, uint8_t tid)
{
  ...
  if (m_qosSupported)
  ...
  else
    {
      ...
      m_dca->Queue (packet, hdr, mcs_map);
    }
...
{  
  //std::cout << "Access point sending one beacon" <<std::endl;
  NS_LOG_FUNCTION (this);
  WifiMacHeader hdr;
  ...
  if (m_s1gSupported) {
  ...
  else if (m_pageslice.GetPageSliceCount () == 0 && numPagedStas == 0)
   {
    ...
    //std::cout << "m_PageSliceNum: " << m_PageSliceNum << std::endl;
    //std::cout << "PageSliceCount: " << m_pageslice.GetPageSliceCount() - 1 << std::endl;
    if (m_PageSliceNum != (m_pageslice.GetPageSliceCount() - 1) && m_PageSliceNum != 31) // convenient overflow if count==0
      {
        //std::cout << "setting number of encoded block" << std::endl;
  ...
  //std::cout << "pageBitmap length: " << unsigned(m_pageslice.GetPageBitmapLength()) << std::endl;
  if (m_pageslice.GetPageBitmapLength()){
    	//uint8_t numBlocksToEncode = m_pageslice.GetPageBitmapLength();
    //std::cout << "here" << std::endl;
    //std::cout << "NumEncodedBlock: " << NumEncodedBlock << std::endl;
    for (uint8_t i = 0; i< NumEncodedBlock; i++){
      ...
      //std::cout << "=================================AP is setting virtual BitMap=======================" << std::endl;
    ...
    */
    ...
    m_edca.find (AC_BK)->second->SetsleepList (m_sleepList);
      ...
      //Get Predicted mcs
      std::cout << "AP send beacon" <<std::endl;
     
      std::map <Mac48Address, uint16_t> mcs_map = updateMCS(m_packetReceived);
      
      //std::cout << " " << std::endl;
      for (auto it = mcs_map.begin(); it != mcs_map.end(); ++it){
        std::cout << "address: " << it->first << " mcs_index: " << it->second << std::endl;
      } 

      packet->AddHeader (beacon);
      m_beaconDca->Queue (packet, hdr, mcs_map);
...
uint8_t total1 = 0 ;
void
ApWifiMac::Receive (Ptr<Packet> packet, const WifiMacHeader *hdr, std::map <Mac48Address, uint16_t> mcs_map, double signal, double noise, double interference, double mcs_index, double psr)
{
  total1++;
  //std::cout << "total received: " << unsigned(total1) << std::endl;
  ...
  if (hdr->IsData ())
    {
      ...
      if (!hdr->IsFromDs ()
        ...
        {
          //std::cout << "data from: "  << from << std::endl;
                  ...
                  if (hdr->IsQosAmsdu ())
                  ...
                  else
                    {
                      if (packet->GetSize() != 60 && packet->GetSize() != 64) {
                        total++;
                        double sinr = (signal)/(noise+interference);
                        std::cout << "frame from: " << from 
                                  << " signal: " << signal 
                                  << " noise: " << noise 
                                  << " interference: " << interference << std::endl;
                        std::cout << "sinr: " << sinr << std::endl;
                        m_packetReceived[from] = sinr;
                        received_data = true;
                      }
  ...
  RegularWifiMac::Receive (packet, hdr, mcs_map, signal, noise, interference, mcs_index, psr);
}

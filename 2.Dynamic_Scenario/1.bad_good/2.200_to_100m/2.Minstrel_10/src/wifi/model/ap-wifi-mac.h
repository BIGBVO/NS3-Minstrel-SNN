...
private:
  virtual void Receive (Ptr<Packet> packet, const WifiMacHeader *hdr, std::map <Mac48Address, uint16_t> mcs_map, double signal, double noise, double interference, double mcs_index, double psr);
  ...
  std::map<Mac48Address, uint16_t> updateMCS(std::map<Mac48Address, double> address_sinr_map);
  ...
  std::map<Mac48Address, uint16_t> m_MacAddrToAid;
  ...
  std::map<Mac48Address, double> m_packetReceived;

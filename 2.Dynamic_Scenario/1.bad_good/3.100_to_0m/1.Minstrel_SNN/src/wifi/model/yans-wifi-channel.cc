...
void
YansWifiChannel::SetChecker (uint16_t check)
{
  m_check = check;
}

uint16_t
YansWifiChannel::GetChecker (void)
{
  return m_check;
}
...
void
YansWifiChannel::Send (Ptr<YansWifiPhy> sender, Ptr<const Packet> packet, std::map <Mac48Address, uint16_t> mcs_map,
                      double txPowerDbm, WifiTxVector txVector, WifiPreamble preamble, uint8_t packetType, Time duration) const
{
  /*
  std::cout << "Channel Send: " << std::endl;
      ...
      if (sender != (*i))
        {
          ...
          *(atts + 2) = duration.GetNanoSeconds ();
          ...
          Data new_data;
          new_data.j = j;
          new_data.mcs_map = mcs_map;

          Simulator::ScheduleWithContext (dstNode, delay, &YansWifiChannel::Receive, this,
                                          new_data, copy, atts, txVector, preamble);
        }
    }
}

void
YansWifiChannel::Receive ( Data data, Ptr<Packet> packet,
                           double *atts, WifiTxVector txVector, WifiPreamble preamble) const
{
  /*
  std::cout << "channel receive" << std::endl;
  for (auto it = data.mcs_map.begin(); it != data.mcs_map.end(); ++it){
      std::cout << "address: " << it->first << std::endl;
  } 
  */
  m_phyList[data.j]->StartReceivePreambleAndHeader (packet, data.mcs_map, *atts, txVector, preamble, *(atts + 1), NanoSeconds (*(atts + 2)));
  delete[] atts;
}

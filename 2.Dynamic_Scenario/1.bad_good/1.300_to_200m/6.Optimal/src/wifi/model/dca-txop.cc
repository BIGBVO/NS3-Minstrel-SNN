...
void
DcaTxop::Queue (Ptr<const Packet> packet, const WifiMacHeader &hdr, const std::map <Mac48Address, uint16_t> mcs_map)
{
  NS_LOG_FUNCTION (this << packet << &hdr);
  //std::cout << "map address: " << &mcs_map << std::endl;
  ...
  m_queue->Enqueue (packet, hdr, mcs_map);
  StartAccessIfNeeded ();
}
...
DcaTxop::NotifyAccessGranted (void)
{
  ...
  if (m_currentPacket == 0)
  {
      ...
      m_currentPacket = m_queue->Dequeue (&m_currentHdr, &mcs_map);
...
/*
          ...
          Low ()->StartTransmission (fragment, &hdr, mcs_map, params,
                                     m_transmissionListener);
          //nrOfTransmissionsDuringRaw++;
        }
      else
...
/*
          ...
          Low ()->StartTransmission (m_currentPacket, &m_currentHdr, mcs_map,
...
DcaTxop::StartNext (void)
{
  ...
  Low ()->StartTransmission (fragment, &hdr, m_mcs_map, params, m_transmissionListener);
  //nrOfTransmissionsDuringRaw++;
}

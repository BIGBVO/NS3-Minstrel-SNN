...
void
YansWifiPhy::SendPacket (Ptr<const Packet> packet, std::map <Mac48Address, uint16_t> mcs_map, WifiTxVector txVector, WifiPreamble preamble, uint8_t packetType)
{
  /*
  std::cout << "Yans wifi phy send packet" <<std::endl;
  for (auto it = mcs_map.begin(); it != mcs_map.end(); ++it){
    std::cout << "address: " << it->first << std::endl;
  } 
  */
  ...
  m_channel->Send (this, packet, mcs_map, GetPowerDbm (txVector.GetTxPowerLevel ()) + m_txGainDb, txVector, preamble, packetType, txDuration);
}
...
uint32_t totala = 0;
void
YansWifiPhy::EndReceive (Ptr<Packet> packet, std::map <Mac48Address, uint16_t> mcs_map, enum WifiPreamble preamble, uint8_t packetType, Ptr<InterferenceHelper::Event> event)
{
  /*
  std::cout << "YansWifiPhy end receive" << std::endl;
  for (auto it = mcs_map.begin(); it != mcs_map.end(); ++it){
      std::cout << "address: " << it->first << std::endl;
  } 
  */
  ...
  //std::cout << "phy layer interference: " << snrPer.snr_comp.interference << std::endl;
  m_interference.NotifyRxEnd ();
  ...
  if (m_plcpSuccess == true)
    {
      ...
      double psr = 1 - snrPer.per;
      std::string path = "/home/sdn01/Desktop/Vincent/Test/RCA_Orig_bad_good/RCA-DNN/NS3_80211.ah_AI_Minstrel/Result/log.txt";

      uint8_t bw;
      if (snrPer.snr_comp.mcs_index <=9){
        bw = 1;
      } else if (snrPer.snr_comp.mcs_index <= 18){
        bw = 2;
      } else {
        bw = 4;
      }

      double snr = bw * snrPer.snr_comp.signal / (snrPer.snr_comp.noise + snrPer.snr_comp.interference);
      snrPer.snr_comp.signal = bw * snrPer.snr_comp.signal;
      snrPer.snr = bw * snrPer.snr;

      if (0.5 >= snrPer.per)
        {
          if (packet->GetSize() == 166){
            /*
            std::cout << "Yans Wifi Phy received size: " << packet->GetSize() << 
            " signal: " << snrPer.snr_comp.signal << " noise: " << snrPer.snr_comp.noise << 
            " interference: " << snrPer.snr_comp.interference << " mcs: " << snrPer.snr_comp.mcs_index <<
            " psr: " << psr << std::endl;
            totala += 1;
            */
            ...
            outfile << snr;
            outfile << "\t";
            outfile << snrPer.snr_comp.mcs_index;
            outfile << "\t";
            outfile << 1;
            outfile << "\n";
            outfile.close();
          ...
          double psr = 1 - snrPer.per;
          m_state->SwitchFromRxEndOk (packet, mcs_map, snrPer.snr_comp.signal, snrPer.snr_comp.noise, snrPer.snr_comp.interference, snrPer.snr_comp.mcs_index, psr, event->GetTxVector (), event->GetPreambleType ());

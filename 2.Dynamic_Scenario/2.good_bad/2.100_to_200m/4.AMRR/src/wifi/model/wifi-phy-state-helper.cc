...
void
WifiPhyStateHelper::SwitchFromRxEndOk (Ptr<Packet> packet,std::map <Mac48Address, uint16_t> mcs_map, double signal, double noise, double interference, double mcs_index, double psr, WifiTxVector txVector, enum WifiPreamble preamble)
{
  double snr = snr/(noise+interference);
  m_rxOkTrace (packet, snr, txVector.GetMode (), preamble);
  NotifyRxEndOk ();
  DoSwitchFromRx ();
  if (!m_rxOkCallback.IsNull ())
    {
      m_rxOkCallback (packet, mcs_map, signal, noise, interference, mcs_index, psr, txVector, preamble);
    }

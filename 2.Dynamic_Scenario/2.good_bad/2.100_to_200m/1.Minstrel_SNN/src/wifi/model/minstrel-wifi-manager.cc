...
MinstrelWifiManager::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::MinstrelWifiManager")
    ...
    .AddAttribute ("Bandwidth",
                   "Channel Bandwidth",
                   DoubleValue (4),
                   MakeDoubleAccessor (&MinstrelWifiManager::bandwidth),
                   MakeDoubleChecker <double> ())
  ;
  return tid;
}
...
MinstrelWifiManager::SetupPhy (Ptr<WifiPhy> phy)
{
  ...
  //std::cout << "Setting Out physical Layer" << std::endl;
  ...
  if (!station->m_initialized && GetNSupported (station) > 1)
    {
      ...
      //m_mcsdl->SetBandwidth(bandwidth);
...
void
MinstrelWifiManager::DoReportRxOk (WifiRemoteStation *st, uint16_t mcs_index,
                                   double rxSnr, WifiMode txMode)
{
  //temp to check predicted index from DNN
  if (mcs_index >= 0 && mcs_index < 30){
    MinstrelWifiRemoteStation *station = (MinstrelWifiRemoteStation *) st;
    //station->m_predict = mcs_index;
    m_p = mcs_index;
    std::cout << "Minstrel set Predicted Rate called" << std::endl;
    std::cout << "======================================================predicted mcs_index= " << mcs_index << std::endl;
  } else {
    std::cout << "||||||||||||||||||||||||||||||||||||||||||||||||||||||| " << mcs_index << std::endl;
  }
...
{
  //std::cout << "Minstrel Report Data OK" << std::endl;
  //std::cout << "Ack SNR: " << ackSnr << " Data SNR: " << dataSnr << std::endl;
  ...
  if (m_nsupported >= 1)
    {
      station->m_txrate = FindRate (station, dataSnr);
    }
}
...
MinstrelWifiManager::DoReportFinalDataFailed (WifiRemoteStation *st)
{
  ...
  if (m_nsupported >= 1)
    {
      station->m_txrate = FindRate (station, 0.01);
    }
}
...
{
  //std::cout << "Minstrel Get Next Sample" << std::endl;
  uint32_t bitrate;
...
uint32_t
MinstrelWifiManager::FindRate (MinstrelWifiRemoteStation *station, double snr)
{
  NS_LOG_FUNCTION (this << station);
  //std::cout << "Minstrel Finding Rate" << std::endl;
  ...
  if ( (((100 * station->m_sampleCount) / (station->m_sampleCount + station->m_packetCount )) < m_lookAroundRate)
       && (coinFlip == 1) )
    {
      ...
      std::cout << "minstrel find rate sampling rate: " << idx << std::endl;
      
     
      //std::cout << "Look Around rate index: " << idx << std::endl;
      /**
      ...
      for (uint32_t retries = 2; retries < 11; retries++)
        {
          ...
          if (totalTxTimeWithGivenRetries > retries*MilliSeconds (6))
            {
              break;
            }

...
StaWifiMac::SendPspoll (void)
{
  ...
  m_pspollDca->Queue (packet, hdr, mcs_map);
}
...
StaWifiMac::SendPspollIfnecessary (void)
{
  ...
  if ( m_rawStart & m_inRawGroup && m_pagedStaRaw && m_dataBuffered )
  ...
  else if (!m_rawStart && m_dataBuffered && !m_outsideRawEvent.IsRunning ()) //in case the next beacon coming during RAW, could it happen?
   {
	...
	{
    //std::cout << "///////////////////////Station recieved TIM///////////////////" << std::endl;
		m_TIM = beacon.GetTIM();
		...
		if (m_TIM.GetDTIMCount() == 0){
			m_pagedInDtim = false;
      //std::cout << "station [aid=" << GetAID() << "] received DTIM beacon." <<std::endl;
			...
			if (m_selfBlock < m_BlockOffset)
			{
				...
				NS_LOG_DEBUG("[aid=" << GetAID() << "] belongs to a block " << (int)m_selfBlock << " which is not included in current page (" << (int)m_selfPage << ") because block offset in Page Slice element is " << (int)m_BlockOffset << " Scheduling sleep until the next DTIM beacon.");
				m_pagedInDtim = false;
      ...
      for (uint8_t i = 0; i < m_PageBitmapLen; i++)
			...
			NS_LOG_DEBUG("[aid=" << this->GetAID() << "] received TIM" << (int)m_TIM.GetDTIMCount () << " beacon.");
		}
		/*if ((m_DTIMCount == 0 && m_TIMOffset ==0) || (m_DTIMCount !=0 && (m_DTIMPeriod - m_DTIMCount) == m_TIMOffset) )
		 // page slice start from m_TIMOffset
		 {
		 if (m_se standard is not clear on the correct queue for management
  //frames if we are a QoS AP. The approach taken here is to always
  //use the DCF for these regardless of wheelfBlock >= m_BlockOffset && m_selfPage == m_Pageindex_slice)
		 ...
		 {
		 BeaconNe standard is not clear on the correct queue for management
  //frames if we are a QoS AP. The approach taken here is to always
  //use the DCF for these regardless of wheumForTIM = m_selfBlockPos/m_PageSliceLen;
    ...
    //std::cout << "*****************************************88Station information page Size: " << unsigned(m_TIM.GetInformationFieldSize()) << std::endl;
...
void
StaWifiMac::Receive (Ptr<Packet> packet, const WifiMacHeader *hdr, std::map <Mac48Address, uint16_t> mcs_map, double signal, double noise, double interference, double mcs_index, double psr)
{
  /*
  std::cout << "Sta wifi mac receive" << std::endl;
  ...
  */
  ...
  if (hdr->GetAddr3 () == GetAddress ())
    ...
    if (goodBeacon)
     {
      //std::cout << "sta wifi mac receiving good beacon" << std::endl;
      WifiMacHeader currentHdr = *hdr; 
      WifiRemoteStation *station = m_stationManager->GetStation(GetAddress(), &currentHdr);
      for (auto it = mcs_map.begin(); it != mcs_map.end(); ++it){
        //std::cout << "address: " << it->first << " mcs_index: " << it->second << std::endl;
        if (GetAddress() == it->first){
          std::cout << "address: " << it->first << " match" << std::endl;
          m_stationManager->SetPredictedRate(station, it->second);
        }
      } 

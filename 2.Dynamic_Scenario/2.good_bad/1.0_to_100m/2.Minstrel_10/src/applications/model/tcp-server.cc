...
void TcpServer::ReceivePacket(Ptr<Socket> s) {
	...
	while (packet = s->RecvFrom(from)) {
		if (packet->GetSize() > 0) {
			...
			total_byte = total_byte + packet->GetSize ();
			std::cout << "Server Received " << packet->GetSize () << " bytes from " << InetSocketAddress::ConvertFrom (from).GetIpv4 () << " total receive: " << unsigned(receive) << std::endl;
			std::cout << "Total byte: " << total_byte << std::endl;

...
WifiRemoteStation* 
WifiRemoteStationManager::GetStation (Mac48Address address, const WifiMacHeader *header){
  WifiRemoteStation *station = Lookup(address, header);
  return station;
}
...
void
WifiRemoteStationManager::SetPredictedRate (WifiRemoteStation *station, uint16_t mcs_index){
  //<<"wifi remote station manager called" << std::endl;
  //std::cout<<"mcs_index: " << mcs_index << std::endl;
  WifiMode wfmode;
  DoReportRxOk(station, mcs_index, 0, wfmode);
}
                                      ...
                                      double rxSnr, WifiMode txMode)
{
  ...
  WifiRemoteStation *station = Lookup (address, header);
  uint16_t mcs_index = -1;
  DoReportRxOk (station, mcs_index, rxSnr, txMode);
}
...
{
  //std::cout << "get supported: " << i << std::endl;
  if (i < 0){
    i = 0;
  }

...
public:
  ...
  virtual void SendPacket (Ptr<const Packet> packet, std::map <Mac48Address, uint16_t> mcs_map, WifiTxVector txvector, enum WifiPreamble preamble, uint8_t packetType);
...
void Configure80211ah (void);
   ...
   * \param event the corresponding event of the first time the packet arrives
   */
  void EndReceive (Ptr<Packet> packet,std::map <Mac48Address, uint16_t> mcs_map, enum WifiPreamble preamble, uint8_t packetType, Ptr<InterferenceHelper::Event> event);

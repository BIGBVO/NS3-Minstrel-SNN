...
void
WifiMacQueue::Enqueue (Ptr<const Packet> packet, const WifiMacHeader &hdr, const std::map <Mac48Address, uint16_t> mcs_map)
{
  Cleanup ();
  if (m_size == m_maxSize)
    {
  ...
  m_queue.push_back (Item (packet, hdr, now, mcs_map));
  m_size++;
}
...
Ptr<const Packet>
WifiMacQueue::Dequeue (WifiMacHeader *hdr, std::map <Mac48Address, uint16_t> *mcs_map)
{
  Cleanup ();
  if (!m_queue.empty ())
    {
      ...
      *hdr = i.hdr;
      *mcs_map = i.mcs_map;
      //std::cout << "Dequeue map pointer address: " << &mcs_map << std::endl;
...
WifiMacQueue::PushFront (Ptr<const Packet> packet, const WifiMacHeader &hdr)
{
  ...
  m_queue.push_front (Item (packet, hdr, now, mcs_map));
  m_size++;
}

...
uint32_t total_prev_successful_transmission = 0;
...
void PrintPositionsFirst (NodeContainer wifiStaNode)
{
...
void PrintPositions (NodeContainer wifiStaNode)
{
	...
	for (int i = 0; i < config.Nsta; i++){
    	...
    	Vector pos = mob->GetPosition ();
    	 std::cout << "=============================================== Station: " << i << " POS: x=" << pos.x << ", y=" << pos.y << ", z=" << pos.z << "," << Simulator::Now ().GetSeconds ()<< std::endl;
		mob->SetVelocity (Vector(1,0,0));
	}
	...
	*/
	...
	uint32_t totalSuccessfulPackets = 0;
	...
	//float snr = (pow(10, ((0-(8+37.6*log10(pos.x)+3))/10) - 3))/(3.83*pow(10, -14))*1.98;

	//Printing Out to file

	std::ofstream outfile;
	outfile.open(config.resultPath, std::ios_base::app);
	if (!outfile.is_open()){
	...
	std::string mcs;
	string dataMode = config.DataMode;
	...
	uint32_t time = Simulator::Now().GetSeconds() - 10;
	outfile << time;
	outfile << "\t";
	outfile << totalSentPackets;
	outfile << "\t";
	outfile << totalSuccessfulPackets;
	outfile << "\t";
	outfile << (totalSuccessfulPackets - total_prev_successful_transmission)*800/1000000.00; //instant throughput
	outfile << "\t";
	outfile << (totalSuccessfulPackets)*800/1000000.00/time; //average tput
	outfile << "\n";

	total_prev_successful_transmission = totalSuccessfulPackets;
...
void checkRawAndTimConfiguration (void)
{
	...
	std::cout << "size" << config.rps.rpsset.size() << std::endl;
    ...
    if(config.pageSliceCount)
    {
	std::cout << "pageSliceCount: " << config.pageSliceCount << std::endl;
	std::cout << "rps size: " << config.rps.rpsset.size() << std::endl;
...
bool check (uint16_t aid, uint32_t index)
{
	...
	std::cout << "AID: " << unsigned(aid) << std::endl;
	std::cout << "toTIm: " << unsigned(toTim) << std::endl;
	return toTim == index;
}
...
void configureTCPSensorClients() {
	...
	cout << "=================================================configure TCP snesor Clients" << endl;
	cout << "Interval: " << MilliSeconds(config.trafficInterval) << endl;
	...
	for (std::map<uint16_t, float>::iterator it = traffic_sta.begin(); it != traffic_sta.end(); ++it) {
		...
		factory.Set("Interval", TimeValue(Time(intervalsta)));

		Ptr<Application> tcpClient = factory.Create<TCPSensorClient>();
		wifiStaNode.Get(it->first)->AddApplication(tcpClient);
		auto clientApp = ApplicationContainer(tcpClient);
		wireTCPClient(clientApp, it->first);

		double random = m_rv->GetValue(0,  ((config.payloadSize * 8) / (it->second * 1000000)));
		cout << "station: " << it->first << " random: " << random << endl; 
		clientApp.Start(MilliSeconds(0 + random));
		clientApp.Stop(Seconds(config.simulationTime));
	}
}
...
void configureUDPClients() {
	...
	if (trafficfile.is_open()) {
		...
		for (uint16_t kk = 0; kk < config.Nsta; kk++) {
			...
			traffic_sta.insert(std::make_pair(sta_id, sta_traffic)); //insert data
			cout << "sta_id = " << sta_id << " sta_traffic = " << sta_traffic << "\n";
			cout << "current_rate = " << config.DataMode << "\n";
		}
		trafficfile.close();
	} else
		cout << "Unable to open traffic file \n";
...
int main(int argc, char *argv[]) {
	...
	LogComponentEnable ("MinstrelWifiManager", LOG_DEBUG);
     //LogComponentEnable ("UdpClient", LOG_INFO);
		 //LogComponentEnable ("UdpServer", LOG_INFO);
		 //LogComponentEnable ("MacLow", LOG_DEBUG);
	...
	//LogComponentEnable ("InterferenceHelper", LOG_DEBUG);
  ...
  phy.Set ("ChannelWidth", UintegerValue (4));
	...
	for (uint16_t i = 0; i < config.Nsta; i++) {
		...
		/*
			...
			position->Add (Vector (-30, -60, 0));
		} else if (i == 9){
			position->Add (Vector (-60, -60, 0));
		} else if (i == 10){
			position->Add (Vector (-60, -30, 0));
		} else if (i == 11) {
			position->Add (Vector (-30, -30, 0));
		} else if (i == 12) {
			position->Add (Vector (-30, 30, 0));
		} else if (i == 13) {
			position->Add (Vector (-60, 30, 0));
	...
	*/
    ...
    mobilityAp.SetMobilityModel("ns3::ConstantPositionMobilityModel");
    mobilityAp.Install(wifiApNode);
	PrintPositionsFirst(wifiStaNode);
	*/
	/*
	...
	sendStatistics(true);
	Simulator::Schedule(Seconds(11), &PrintPositions, wifiStaNode);
	...
	std::cout << "totalSendPackets: " << totalSentPackets << std::endl;
	std::cout << "totalSuccessfulPackets: " << totalSuccessfulPackets << std::endl;
	std::cout << "packetSuccess: " << (double(totalSuccessfulPackets)/totalSentPackets)*100 << "%" << std::endl;
	std::cout << "" << std::endl;
...
*/
	...
	for (int i = 0; i < config.Nsta; i++){
		Ptr<ConstantPositionMobilityModel> mob = wifiStaNode.Get(i)->GetObject<ConstantPositionMobilityModel>();
    	Vector pos = mob->GetPosition ();
    	std::cout << "Station: " << i << " POS: x=" << pos.x << ", y=" << pos.y << ", z=" << pos.z << std::endl;
		//mob->SetVelocity (Vector(0,0,0));
	}

	NS_ASSERT(false);
	double throughput = (totalSuccessfulPackets) * config.payloadSize * 8 / (config.simulationTime * 1000000.0);
	...
	{
		double throughput = 0;
		double totalPacketsThrough = (config.payloadSize * 8) / (config.simulationTime * 1000000.0);
		...
		/*
		std::ofstream outfile;
		outfile.open("/home/vo/Desktop/Wi-Fi_Halow_v3/IEEE-802.11ah-ns-3/result/minstrel-EWMA/1-RAW-4-Stations/1ms-0-500.txt", std::ios_base::app);
		if (!outfile.is_open()){
			std::cout << "Failed" <<std::endl;
		} else{
		std::cout <<"File success" << std::endl;

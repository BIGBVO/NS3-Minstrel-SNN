...
InterferenceHelper::InterferenceHelper ()
  : m_errorRateModel (0),
    m_firstPower (0.0),
    m_rxing (false),
    inter_start (Simulator::Now ())
{
 
  //std::cout << "time: " << inter_start << std::endl;
  //std::cout << "============================here interference helper being set up================================" << std::endl;
}
...
struct InterferenceHelper::SNR_comp
InterferenceHelper::CalculateSnr (double signal, double noiseInterference, WifiMode mode) const
{
  ...
  double change_factor = 0.5;
  Time time_passed = Simulator::Now () - inter_start ;
  // std::cout << "============================================ time passed: " << time_passed << " =====================================" << std::endl;
  double signal_changes = signal*(100-0.5);
  double snr = signal  / noise;
  NS_LOG_DEBUG ("signal= " << signal << ", noise=" << noiseFloor << ", interference=" << noiseInterference << ", snr=" << snr);
  //return snr;
  struct SNR_comp comp;
  comp.signal = signal;
  comp.noise = noiseFloor;
  comp.interference = noiseInterference;

  //std::cout << "comp.interference: " << comp.interference << std::endl;
...
double
InterferenceHelper::CalculateChunkSuccessRate (InterferenceHelper::SNR_comp snr_comp, Time duration, WifiMode mode) const
{
  double snir = (snr_comp.signal)/(snr_comp.noise + snr_comp.interference);
...
InterferenceHelper::PerInterference
InterferenceHelper::CalculatePlcpPayloadPer (Ptr<const InterferenceHelper::Event> event, NiChanges *ni) const
{
 ...
 if (payloadMode.GetModulationClass () != WIFI_MOD_CLASS_S1G)
 {
  ...
  //std::cout << "noiseInterfereneceW: " << noiseInterferenceW << std::endl;

  double mcs_index;

  if (payloadMode.GetUniqueName() == "OfdmRate300KbpsBW1MHz")
		mcs_index = 0;
	else if (payloadMode.GetUniqueName() == "OfdmRate600KbpsBW1MHz")
		mcs_index = 1;
	else if (payloadMode.GetUniqueName() == "OfdmRate900KbpsBW1MHz")
		mcs_index = 2;
	else if (payloadMode.GetUniqueName() == "OfdmRate1_2MbpsBW1MHz")
		mcs_index = 3;
	else if (payloadMode.GetUniqueName() == "OfdmRate1_8MbpsBW1MHz")
		mcs_index = 4;
	else if (payloadMode.GetUniqueName() == "OfdmRate2_4MbpsBW1MHz")
		mcs_index = 5;
	else if (payloadMode.GetUniqueName() == "OfdmRate2_7MbpsBW1MHz")
		mcs_index = 6;
	else if (payloadMode.GetUniqueName() == "OfdmRate3MbpsBW1MHz")
		mcs_index = 7;
	else if (payloadMode.GetUniqueName() == "OfdmRate3_6MbpsBW1MHz")
		mcs_index = 8;
	else if (payloadMode.GetUniqueName() == "OfdmRate4MbpsBW1MHz")
		mcs_index = 9;

	else if (payloadMode.GetUniqueName() == "OfdmRate650KbpsBW2MHz")
		mcs_index = 10;
	else if (payloadMode.GetUniqueName() == "OfdmRate1_3MbpsBW2MHz")
		mcs_index = 11;
	else if (payloadMode.GetUniqueName() == "OfdmRate1_95MbpsBW2MHz")
		mcs_index = 12;
	else if (payloadMode.GetUniqueName() == "OfdmRate2_6MbpsBW2MHz")
		mcs_index = 13;
	else if (payloadMode.GetUniqueName() == "OfdmRate3_9MbpsBW2MHz")
		mcs_index = 14;
	else if (payloadMode.GetUniqueName() == "OfdmRate5_2MbpsBW2MHz")
		mcs_index = 15;
	else if (payloadMode.GetUniqueName() == "OfdmRate5_85MbpsBW2MHz")
		mcs_index = 16;
	else if (payloadMode.GetUniqueName() == "OfdmRate6_5MbpsBW2MHz")
		mcs_index = 17;
	else if (payloadMode.GetUniqueName() == "OfdmRate7_8MbpsBW2MHz")
		mcs_index = 18;

	else if (payloadMode.GetUniqueName() == "OfdmRate1_35MbpsBW4MHz")
		mcs_index = 19;
	else if (payloadMode.GetUniqueName() == "OfdmRate2_7MbpsBW4MHz")
		mcs_index = 20;
	else if (payloadMode.GetUniqueName() == "OfdmRate4_05MbpsBW4MHz")
		mcs_index = 21;
	else if (payloadMode.GetUniqueName() == "OfdmRate5_4MbpsBW4MHz")
		mcs_index = 22;
	else if (payloadMode.GetUniqueName() == "OfdmRate8_1MbpsBW4MHz")
		mcs_index = 23;
	else if (payloadMode.GetUniqueName() == "OfdmRate10_8MbpsBW4MHz")
		mcs_index = 24;
	else if (payloadMode.GetUniqueName() == "OfdmRate12_15MbpsBW4MHz")
		mcs_index = 25;
	else if (payloadMode.GetUniqueName() == "OfdmRate13_5bpsBW4MHz")
		mcs_index = 26;
	else if (payloadMode.GetUniqueName() == "OfdmRate16_2MbpsBW4MHz")
		mcs_index = 27;
	else if (payloadMode.GetUniqueName() == "OfdmRate18MbpsBW4MHz")
		mcs_index = 28;

  //std::cout << "Modulation index: " << mcs_index << std::endl;
  double per = 1 - psr;
  struct PerInterference perInterference;
  perInterference.per = per;
  perInterference.interference = noiseInterferenceW;
  perInterference.mcs_index = mcs_index;
  return perInterference;
}
...
InterferenceHelper::CalculatePlcpPayloadSnrPer (Ptr<InterferenceHelper::Event> event)
{
  ...
  struct SNR_comp snr_comp_temp = CalculateSnr (event->GetRxPowerW (),
                             noiseInterferenceW,
                             event->GetPayloadMode ());


  double snr = (snr_comp_temp.signal)/(snr_comp_temp.noise + snr_comp_temp.interference);
  
  struct SnrPer snrPer;
  snrPer.snr = snr;
  snrPer.snr_comp = snr_comp_temp;
  
  //std::cout << "snr_comp.interference: " << snr_comp_temp.interference << std::endl;
  /* calculate the SNIR at the start of the packet and accumulate
   * all SNIR changes in the snir vector.
   */

  struct PerInterference perInterferenceTemp;
  perInterferenceTemp = CalculatePlcpPayloadPer (event, &ni);
  snrPer.per = perInterferenceTemp.per;
  snrPer.snr_comp.interference = perInterferenceTemp.interference;
  snrPer.snr_comp.mcs_index = perInterferenceTemp.mcs_index;
  //std::cout << "returning here" << std::endl;
  //std::cout << "interference: " << snrPer.snr_comp.interference << std::endl;
  return snrPer;
}
...
InterferenceHelper::CalculatePlcpHeaderSnrPer (Ptr<InterferenceHelper::Event> event)
{
  NiChanges ni;
  double noiseInterferenceW = CalculateNoiseInterferenceW (event, &ni);
  struct SNR_comp snr_comp = CalculateSnr (event->GetRxPowerW (),
                             noiseInterferenceW,
  ...
  double snr = (snr_comp.signal)/(snr_comp.noise + snr_comp.interference);

  struct SnrPer snrPer;
  snrPer.snr = snr;
  snrPer.per = per;
  snrPer.snr_comp = snr_comp;
  return snrPer;
}
